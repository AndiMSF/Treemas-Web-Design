...
const MasterDataPermission = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "http://192.168.2.55:8081/api/master-data/permission-view",
          {
  ...
  const handleClick = (id) => {
    const selectedPermission = apiData.find(
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await fetch(
            `http://192.168.2.55:8081/api/master-data/permission-form/delete/${id}`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
  ...
  return (
    <div className="content__container">
      ...
      <Link to="/master-data/permission-form/add" className="add__button">
        Tambah
      </Link>
